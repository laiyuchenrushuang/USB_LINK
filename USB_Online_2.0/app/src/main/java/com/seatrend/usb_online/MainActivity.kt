...
class MainActivity : BaseActivty(), MyAdapter.Dback {
    ...
    private var mData = ArrayList<DataEnity.DATA>()
                ...
                Constants.GET_DATA -> {
                    ...
                    try {
                        if (null != msg.obj) {
                            ...
                            showToast("receive data  is null.")
                        }
                    } catch (e: Exception) {
                        showToast(e.message!!)
                    }
                        ...
                        if (Constants.READ_FAIL != mDecodeResult.toString() && !TextUtils.isEmpty(mDecodeResult.toString())) {
                            for (index in 0 until mData.size) {
                                ...
                                if (index == mData.size - 1) {//最后一个了
                                    showToast("The scanned barcode is not in the list.")
                                }
                            }
                            adapter!!.setData(mData)
                        } else {
                            showToast("SCAN_READ_FAIL")
                        }
